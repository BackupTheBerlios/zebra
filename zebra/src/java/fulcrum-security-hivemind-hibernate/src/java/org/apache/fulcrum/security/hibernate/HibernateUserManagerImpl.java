 ...
 * @version $Id: HibernateUserManagerImpl.java,v 1.1 2005/11/15 17:58:11 bgidley Exp $
 */
public class HibernateUserManagerImpl extends AbstractUserManager {
    ...
    public boolean checkExists(String userName) throws DataBackendException {
        ...
        try {
            users = getPersistenceHelper().retrieveSession().find(
                    "from " + User.class.getName() + " su where su.name=?", userName, Hibernate.STRING);
    ...
    public User getUser(String userName) throws UnknownEntityException, DataBackendException {
        ...
        try {
            users = getPersistenceHelper().retrieveSession().find(
                    "from " + User.class.getName() + " su where su.name=?", userName.toLowerCase(), Hibernate.STRING);
        } catch (HibernateException e) {
            throw new DataBackendException("Error retriving user information", e);
        }
        if (users.size() > 1) {
    ...
    public UserSet getAllUsers() throws DataBackendException {
        ...
        try {
            ...
            List users = getPersistenceHelper().retrieveSession().find("from " + User.class.getName() + "");
            userSet.add(users);
        } catch (HibernateException e) {
    ...
    public User getUserById(Object id) throws DataBackendException, UnknownEntityException {
        ...
        if (id != null)
            try {
                List users = getPersistenceHelper().retrieveSession().find(
                        "from " + User.class.getName() + " su where su.id=?", id, Hibernate.LONG);
                if (users.size() == 0) {
