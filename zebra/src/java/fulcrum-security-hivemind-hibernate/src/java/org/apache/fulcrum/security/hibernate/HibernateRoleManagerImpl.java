 ...
 * @version $Id: HibernateRoleManagerImpl.java,v 1.1 2005/11/15 17:58:11 bgidley Exp $
 */
public class HibernateRoleManagerImpl extends AbstractRoleManager {
    ...
    public boolean checkExists(String roleName) throws DataBackendException {
        ...
        try {
            ...
            roles = getPersistenceHelper().retrieveSession().find(
                    "from " + Role.class.getName() + " sr where sr.name=?", roleName, Hibernate.STRING);
    ...
    public Role getRoleById(Object id) throws DataBackendException, UnknownEntityException {
        ...
        if (id != null)
            try {
                List roles = getPersistenceHelper().retrieveSession().find(
                        "from " + Role.class.getName() + " sr where sr.id=?", id, Hibernate.LONG);
                if (roles.size() == 0) {
