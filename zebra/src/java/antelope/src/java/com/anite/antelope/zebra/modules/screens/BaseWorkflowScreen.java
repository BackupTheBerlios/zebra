...
public abstract class BaseWorkflowScreen extends VelocityScreen {
	 ...
	 * @param form
	 */
	private FormTool loadPausedFormData(AntelopeTaskInstance taskInstance, FormTool form, Context context) {

		if (context.get(BaseWorkflowAction.KEEP_FORM) == null) {
			if (taskInstance.getPropertySet().containsKey(AntelopeTaskInstance.PAUSED_FORM_DETAILS)) {
				form = setFormTool((FormTool) ((AntelopePropertySetEntry) taskInstance.getPropertySet().get(
					AntelopeTaskInstance.PAUSED_FORM_DETAILS)).getObject(), context);
			}
	 ...
	 */
	protected final boolean isAuthorized(AntelopeTaskInstance taskInstance, Context context)
		...
		if (taskInstance.getTaskOwner() == null) {
			...
			if (taskInstance.getTaskOwner() != UserLocator.getLoggedInUser()) {
				return false;
			}
		}
		return permitted;
	...
	protected void doBuildTemplate(RunData data, Context context) throws Exception {
		...
		if (sessionData == null) {
			...
			log.info("Someone has tried to run a workflow screen without a session:"
		...
		if (isAuthorized(taskInstance, context)) {
			...
			try {
				doBuildTemplate(data, context, sessionData.getTaskInstance(), form);
			} catch (Exception e) {
				log.error(null, e);
				handleException(e);
			}
